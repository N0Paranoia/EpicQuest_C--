...
int World::LoadMedia(SDL_Renderer* Renderer, Tile* tiles[], Mobs* mobs[], Objects* objects[]) {
    //Load Tilemap
    ...
    //Load objectSheet
    if((ObjectsSheetTexture.LoadFromFile(Renderer, "assets/objectsSheet.png")) == 0) {
        cout << "Unable to load Mob Texture! SDL_Error: " << SDL_GetError() << endl;
        return false;
    } else {
	ObjectClips[OBJECT_WEAPON_NONE].x = -TILE_SIZE;
    	ObjectClips[OBJECT_WEAPON_NONE].y = -TILE_SIZE;
    	ObjectClips[OBJECT_WEAPON_NONE].w = 0;
    	ObjectClips[OBJECT_WEAPON_NONE].h = 0;

	ObjectClips[OBJECT_WEAPON_DAGGER].x = 0 * TILE_SIZE;
	ObjectClips[OBJECT_WEAPON_DAGGER].y = 0 * TILE_SIZE;
 	ObjectClips[OBJECT_WEAPON_DAGGER].w = 1 * TILE_SIZE;
    	ObjectClips[OBJECT_WEAPON_DAGGER].h = 1 * TILE_SIZE;

	ObjectClips[OBJECT_WEAPON_SWORD].x = 1 * TILE_SIZE;
	ObjectClips[OBJECT_WEAPON_SWORD].y = 0 * TILE_SIZE;
 	ObjectClips[OBJECT_WEAPON_SWORD].w = 1 * TILE_SIZE;
    	ObjectClips[OBJECT_WEAPON_SWORD].h = 1 * TILE_SIZE;
    }
    //Load tilesheet
...
bool World::SetObjects(Objects* objects[]) {
    int x = 0;
    int y = 0;
    return true;
}

void World::Render(SDL_Renderer* Renderer, SDL_Rect* camera, Tile* tiles[], Mobs* mobs[], Objects* objects[], SDL_Rect* player, SDL_Rect* SwordBox) {
    //Render Tiles
    for(int i = 0; i < TOTAL_TILES; i++) {
    ...
    //Render Objects
    for(int i = 0; i < TOTAL_TILES; i++) {
      //objects[i]->Render(&ObjectsSheetTexture, &ObjectClips[Type], Renderer, camera);
	objects[i]->Render(&TileSheetTexture, &TileClips[Type], Renderer, camera);
    }
}
