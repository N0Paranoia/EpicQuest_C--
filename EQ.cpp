...
bool EQ::LoadMedia() {
    ...
    //Load Tile Sheet
    if((world.LoadMedia(Renderer, tileSet, mobs, objects)) == 0) {
        return false;
    }
...
void EQ::Render() {
    ...
    // Render Tiles
    world.Render(Renderer, &camera.cameraRect, tileSet, mobs, objects, &player.playerRect, &player.WeaponBox);
